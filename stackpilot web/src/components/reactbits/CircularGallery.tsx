...
function createTextTexture(
  ...
  canvas.height = Math.max(textHeight + 24, 32);
...
class Media {
  ...
  createShader() {
    ...
    const drawCard = (imageElement?: HTMLImageElement) => {
      if (!ctx) return;
      ctx.clearRect(0, 0, 512, 640);

      // Card background matching #111114
      ctx.fillStyle = '#111114';
      ctx.fillRect(0, 0, 512, 640);

      // Logo rendering in center (prominent, centered, unclipped)
...
class App {
  ...
  planeGeometry!: Plane;
  medias: Media[] = [];
  mediasImages: { image: string; text: string; role?: string }[] = [];
  ...
  addEventListeners() {
    ...
    // Only capture wheel & mousedown directly on the gallery container to keep page scroll buttery smooth
    if (this.container) {
    ...
    // Drag move and release can occur anywhere on window once initiated
