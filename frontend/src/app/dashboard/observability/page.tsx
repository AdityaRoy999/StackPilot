...
type RecentFailure = ObservabilitySummary["recent_failures"][number];
...
export default function ObservabilityPage() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get<ObservabilitySummary>("/observability/summary");
      return response.data;
    },
    refetchInterval: 5000,
  });
  const deploymentsQuery = useQuery({
    queryKey: ["observability-deployments-fallback"],
    queryFn: async () => {
  ...
  const explainFailureMutation = useMutation({
    mutationFn: async (failure: RecentFailure) => {
      const response = await api.post("/ai/chat", {
        message:
          "Explain this deployment failure clearly. Include why it happened, what the GitHub/Git error means, the most likely fix, and the next safe action.\n\nLog excerpt:\n" +
          failure.log_excerpt,
    ...
    <div className="space-y-8">
      <section className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-lg text-muted-foreground">
            Phase 6 monitoring for deployments, queues, runtime health, logs, and platform signals.
          </p>
        </div>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Card>
          <CardHeader>
            ...
            <CardDescription>{data?.service || "aids-backend"}</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <Dialog open={failureExplanationOpen} onOpenChange={setFailureExplanationOpen}>
        <DialogContent className="sm:max-w-3xl">
          ...
          <pre className="max-h-[60vh] overflow-y-auto whitespace-pre-wrap rounded-lg border border-border bg-muted/30 p-4 font-sans text-sm leading-relaxed">
